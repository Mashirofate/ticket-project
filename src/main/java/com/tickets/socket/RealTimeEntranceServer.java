...
@Scope("prototype")
@Component
@ServerEndpoint("/RealTimeEntranceServer/{vaId}")
public class RealTimeEntranceServer {
     ...
     */
    public static ConcurrentHashMap<String, RealTimeEntranceServer> webSocketMap = new ConcurrentHashMap<>();
    /**
    ...
    public static void sendMessage(String vaId, String message) throws IOException {
        if (!StringUtils.isEmpty(vaId) && webSocketMap.containsKey(vaId)) {
            ...
            //System.out.println("实时（绿码）数据开始推送 客户端" + vaId + ",不在线！");
        }
    }

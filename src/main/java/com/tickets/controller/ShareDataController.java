...
public class ShareDataController {
     ...
     */
    @Authentication(isLogin = false,isRequiredUserInfo = false)
    @Operation(summary = "获取开放场馆中所有开放的入口列表")
     ...
     */
    @Authentication(isLogin = false,isRequiredUserInfo = false)
    @Operation(summary = "获取通道列表", description  = "vmId, vmName")
    ...
    @Operation(summary = "查询服务的数据", description  = "根据aId活动ID查询")
